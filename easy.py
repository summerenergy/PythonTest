...
class Data:
    def __init__(self):
        ...
        self.stepsize = 1e-2
        self.train_img_list = np.zeros((self.N, 28 * 28))#生成6000行，28*28矩阵，每一张图片变为一行
        self.train_label_list = np.zeros((self.N, 1))#生成6000行，一列
...
#生成10000行10列矩阵
        ...
        prediction = np.argmax( scores, axis = 1 )#每行最大值的索引
    ...
    def train_network(self, img_batch_list, label_batch_list):
        ...
        # calculate softmax
        train_example_num = img_batch_list.shape[0]#输入的数据
        ...
        scores_e = np.exp( scores )#60000行10列矩阵
        scores_e_sum = np.sum( scores_e, axis = 1, keepdims= True )#按行相加，并保持其行的纬度
        ...
        # backpropagation 损失函数-logx 求导-1/x*ln10

        dscore = np.zeros( (train_example_num, self.K) )#k=10
        for i in range( train_example_num ):
            dscore[ i ][ : ] = probs[ i ][ : ]
